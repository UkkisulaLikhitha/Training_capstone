# -*- coding: utf-8 -*-
"""Indexing_MultiDB_Retrieval_liki.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1xCglEz_Nt9-9c4tSnlRSimKz1xgg1rL5
...
PINECONE_INDEX_NAME = "eytrainingindex"  # auto-created below if missing
...
print(f'Embedding {len(all_docs)} chunks with HuggingFace MiniLM...')
...
faiss_raw.add(vectors_np)

build_ms = (time.perf_counter() - t0) * 1000

print(f"✅ FAISS IndexFlatL2 built in {build_ms:.1f} ms")
print(f"Dimension: {EMBEDDING_DIM}")
print(f"Vectors indexed: {faiss_raw.ntotal}")
...
!pip install -q pinecone sentence-transformers langchain-huggingface
...
# Clean Pinecone Setup
!pip install -q pinecone==7.3.0 langchain-pinecone==0.2.12
...
existing = [x["name"] if isinstance(x, dict) else getattr(x, "name", str(x))
...
"""extension task
 ...
 implement reranking with cohere rerank api compare MRR before/after for top 10 results
"""
...
# Evaluation set: (query, relevant document title)
EVAL_SET = [
    ("How does FAISS perform similarity search?", "FAISS"),
    ("Explain BM25 ranking", "BM25 Information Retrieval"),
    ("How does CRISPR edit DNA?", "CRISPR Gene Editing"),
    ("What caused COVID-19 pandemic?", "COVID-19 Pandemic"),
    ("How do electric vehicles recover energy?", "Electric Vehicles"),
]

# MRR calculation
def reciprocal_rank(docs, relevant_title):
    for rank, doc in enumerate(docs, start=1):
        if relevant_title.lower() in doc.metadata.get("title","").lower():
            return 1.0 / rank
    return 0.0

# Retrieve top 10 FAISS results
def retrieve_top10(query):
    return faiss_store.similarity_search(query, k=10)

# Rerank top 10 results with Cohere
def retrieve_with_rerank(query):
    docs = faiss_store.similarity_search(query, k=10)
    passages = [d.page_content for d in docs]
    reranked = co.rerank(
        model="rerank-v3.5",
        query=query,
...
for query, relevant_doc in EVAL_SET:
    # Before rerank
    before_docs = retrieve_top10(query)
    before_rr = reciprocal_rank(before_docs, relevant_doc)
    before_scores.append(before_rr)

    # After rerank
    after_docs = retrieve_with_rerank(query)
    after_rr = reciprocal_rank(after_docs, relevant_doc)
    after_scores.append(after_rr)

# Mean Reciprocal Rank
mrr_before = np.mean(before_scores)
mrr_after = np.mean(after_scores)

print(f"MRR@10 Before Rerank: {mrr_before:.4f}")
print(f"MRR@10 After  Rerank: {mrr_after:.4f}")
print(f"Improvement: {(mrr_after - mrr_before)*100:.2f}%")
...
plt.ylabel("MRR@10")
plt.title("Impact of Cohere Rerank on FAISS Retrieval")
plt.show()
