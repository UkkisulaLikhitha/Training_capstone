...
Original file is located at
    https://colab.research.google.com/drive/1fLaZfi2zueCRCtMp9NY4H-gJzr-NFyW4
"""
...
# Extension task for symantic based search
# Option C: Search Evaluation Metrics Implement formal information retrieval evaluation.
#  We'll introduce a small ground-truth testing set and calculate MRR (Mean Reciprocal Rank)
# and NDCG (Normalized Discounted Cumulative Gain) to mathematically
# prove how well our search engine performs.

# Extension Task: Evaluate Search using MRR and NDCG


# Ground truth queries and relevant documents
test_queries = {
    "iphone frozen": 0,
    "ios update": 1,
    "samsung camera": 2,
...
for query, relevant_doc in test_queries.items():

    # Get ranked results from hybrid search
    sparse_res = sparse_search(query, top_n=10)
    dense_res = dense_search(query, top_n=10)
    results = reciprocal_rank_fusion(sparse_res, dense_res, top_n=10)
    ...
    # ----- NDCG -----
    relevances = [1 if doc == relevant_doc else 0 for doc in ranked_docs]

    dcg = sum(rel / math.log2(i + 2) for i, rel in enumerate(relevances))
    idcg = 1  # ideal DCG when relevant document is at rank 1

    ndcg_scores.append(dcg / idcg)

# Final Metrics
print("MRR :", round(np.mean(mrr_scores), 4))
print("NDCG:", round(np.mean(ndcg_scores), 4))