# -*- coding: utf-8 -*-
"""CrewAI_Groq_Pipeline.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1NcOe_00dDbpN-r7tmKAwOMgUTFaNtylo
...
monitor_agent = Agent(
    ...
    goal="Detect and summarise supply chain disruptions from incoming alerts.",
    backstory="Experienced logistics analyst who monitors global shipping lanes 24/7 and quickly triages disruption severity.",
    llm=GROQ_SMART,
    verbose=True,
    max_iter=2,
)
...
route_optimiser = Agent(
    role="Route Optimiser",
    goal="Identify the fastest and cheapest alternative shipping routes.",
    backstory="Ex-freight-forwarding specialist with deep knowledge of ports, carriers, and lead times across Europe and Asia.",
    llm=GROQ_SMART,
    verbose=True,
    max_iter=2,
)
...
supplier_comms = Agent(
    role="Supplier Communications Manager",
    goal="Draft clear, professional updates to affected suppliers and customers.",
    backstory="Senior account manager skilled at translating complex logistics problems into concise stakeholder communications.",
    llm=GROQ_SMART,   # changed from GROQ_FAST
    verbose=True,
    max_iter=2,
)
...
compliance_agent = Agent(
    role="Trade Compliance Officer",
    goal="Flag any customs, sanctions, or regulatory issues with alternative routes.",
    backstory="Certified customs broker with expertise in EU/UK/US trade regulations and sanctions screening.",
    llm=GROQ_SMART,
    verbose=True,
    max_iter=2,
)
...
report_agent = Agent(
    ...
    goal="Compile all findings into a concise executive briefing.",
    backstory="Business analyst who distils complex multi-source data into clear, actionable executive summaries.",
    llm=GROQ_SMART,
    verbose=True,
    max_iter=2,
)
...
financial_analyst = Agent(
    ...
    backstory="CFA-qualified financial analyst specialising in logistics cost modelling. Always presents base case, worst case, and best case scenarios.",
    llm=GROQ_SMART,
    verbose=True,
    max_iter=2,
)

# ── Tasks (short expected_output to reduce tokens) ────────────────────────────
task_monitor = Task(
    description="A typhoon has disrupted the Port of Shanghai for 5 days. Three shipments of automotive components are affected. Summarise the disruption: severity, affected shipments, estimated delay.",
    expected_output="3 bullet points: severity level, affected shipments, delay estimate.",
    agent=monitor_agent,
)
...
task_route = Task(
    description="Given the Shanghai port closure, identify 2-3 alternative routing options (e.g. air freight, alternative ports). Include estimated cost and lead time for each.",
    expected_output="Ranked list: route name, cost (EUR), lead time (days). Max 3 options.",
    agent=route_optimiser,
...
task_comms = Task(
    description="Draft a short professional email update to affected suppliers explaining the delay and rerouting plan.",
    expected_output="Email: subject line + max 100 word body + next steps.",
    agent=supplier_comms,
    context=[task_monitor, task_route],
)
...
task_compliance = Task(
    description="Review the alternative routes. Flag customs delays, sanctions exposure, or regulatory requirements for each.",
    expected_output="One line per route: route name + red/amber/green rating + one sentence reason.",
...
task_financial = Task(
    description="Calculate total EUR exposure: rerouting, SLA penalties, insurance.",
    expected_output="Table: 4 cost line items x 3 scenarios (base/worst/best) in EUR.",
    agent=financial_analyst,
    context=[task_monitor, task_route],
)
...
task_report = Task(
    description="Compile all findings into a concise executive briefing: disruption, recommended route, compliance, financials.",
    expected_output="Executive briefing max 200 words with 4 sections.",
    agent=report_agent,
    context=[task_monitor, task_route, task_compliance, task_financial],
)

# ── Crew ──────────────────────────────────────────────────────────────────────
crew = Crew(
    agents=[monitor_agent, route_optimiser, supplier_comms, compliance_agent, financial_analyst, report_agent],
    tasks=[task_monitor, task_route, task_comms, task_compliance, task_financial, task_report],
